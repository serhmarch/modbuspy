#!/usr/bin/env python3
"""
ademoserver.py - Asynchronous Python translation of ModbusLib demoserver.cpp example

This is a complete async Modbus server demonstration that implements:
- TCP, RTU, and ASCII protocols using async patterns
- Full ModbusInterface with simulated memory
- All standard Modbus functions
- Connection monitoring and logging
- Async/await patterns for non-blocking server operations
...
from libmodbuspy.clientport import ModbusAsyncClientPort
...
def parse_arguments() -> Options:
    ...
    parser = argparse.ArgumentParser(
        description="Modbus Async Demo Server - Python version",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  ademoserver.py -t TCP -p 502
  ademoserver.py -u 2 -c 100
        """
    )
...
async def server_process(server):
    """
    Continuously process server operations.
    
    Args:
        server: The Modbus server instance to process
    """
    while True:
        try:
...
async def device_process(device):
    """
    Monitor and update device state with periodic logging.
    
    Args:
        device: The Modbus device to monitor
    """
    tmr = timer()
    while True:
        ...
        # Print register value every second
        if timer() - tmr >= 1000:
            tmr = timer()
            print(f"Register[0] value: {device.memory[0]}")
        
        # Give our loop a short break to prevent CPU overusage
        time.sleep(0.001)
...
async def main():
    ...
    print(f"Protocol: {['ASC', 'RTU', 'TCP', 'UDP', 'RTUvTCP', 'ASCvTCP', 'RTUvUDP', 'ASCvUDP'][options.type]}")
    ...
    if options.type in (ProtocolType.TCP,
                        ProtocolType.RTUvTCP,
                        ProtocolType.ASCvTCP):
        server = ModbusAsyncTcpServer(options.type, device)
        ...
        port = ModbusRtuOverUdpPort(blocking=False)
        port.setPort(options.port)
        port.setTimeout(options.timeout)
        server = ModbusAsyncServerResource(port, device)
    elif options.type == ProtocolType.ASCvUDP:
        port = ModbusAscOverUdpPort(blocking=False)
        port.setPort(options.port)
        port.setTimeout(options.timeout)
        server = ModbusAsyncServerResource(port, device)
    else:
    ...
    print("-" * 50)     
    try:
        print("ademoserver starts ...")
        # Run server processing and device monitoring as parallel tasks
